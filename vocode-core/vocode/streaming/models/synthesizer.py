...
class SarvamSynthesizerConfig(SynthesizerConfig, type="synthesizer_sarvam"):  # type: ignore
    """
    Sarvam AI TTS synthesizer config for Indian languages.
    
    Features:
    - Supports 10 Indian languages + English
    - Natural-sounding Indian voices
    - Configurable pitch, speed, loudness
    - Output: base64 encoded audio
    
    API pricing: ₹15/10K characters
    ...
    """
    api_key: Optional[str] = None
    model: str = DEFAULT_SARVAM_MODEL  # bulbul:v2 or bulbul:v3
    target_language_code: str = DEFAULT_SARVAM_LANGUAGE
    # Voice control (only for bulbul:v2)
    pitch: float = 0.0  # -0.75 to 0.75
    speed: float = 1.0  # 0.3 to 3.0
    loudness: float = 1.0  # 0.3 to 3.0
    # Sample rate (8000, 16000, 22050, 24000, 32000, 44100, 48000 Hz)
    # Higher rates only with bulbul:v3
