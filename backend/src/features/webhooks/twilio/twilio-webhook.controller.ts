...
export class TwilioWebhookController {
  ...
  handleIncomingCall = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      let pipecatSessionId = existingCall?.pipecatSessionId;
      if (!pipecatSessionId) {
        ...
        const pipecatResult = await pipecatService.createConversation({
          callId,
          tenantId,
        ...
        if (pipecatResult.success && pipecatResult.conversationId) {
          pipecatSessionId = pipecatResult.conversationId;
          await this.prisma.call.update({
            where: { id: callId },
            data: { pipecatSessionId },
          });
        }
      }

      const streamUrl = this.resolveStreamUrl(pipecatSessionId);
  ...
  private resolveStreamUrl(pipecatSessionId?: string | null): string | null {
    if (!pipecatSessionId) {
      return null;
    }

    if (env.PIPECAT_STREAM_URL) {
      if (env.PIPECAT_STREAM_URL.includes('{conversationId}')) {
        return env.PIPECAT_STREAM_URL.replace('{conversationId}', pipecatSessionId);
      }
      const separator = env.PIPECAT_STREAM_URL.endsWith('/') ? '' : '/';
      return `${env.PIPECAT_STREAM_URL}${separator}${pipecatSessionId}`;
    }

    // Fallback convention for self-hosted Pipecat pipeline server.
    const wsBase = env.PIPECAT_BASE_URL.replace(/^http:\/\//, 'ws://').replace(
      /^https:\/\//,
      'wss://',
    );
    return `${wsBase}/connect_call/${pipecatSessionId}`;
  }
}
